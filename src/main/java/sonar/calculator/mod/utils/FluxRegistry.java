...
public class FluxRegistry {
    ...
    public static String getNetwork(int networkID) {
        ...
        if (networksNAME.get(networkID) == null || networksNAME.get(networkID)
            .isEmpty()) {
            return "NETWORK";
        }
    ...
    public static void addFlux(IFlux flux) {
        if (flux != null && flux.networkID() != 0) {
            if (flux instanceof IFluxPlug) {
                ...
                if (!plugs.get(flux.networkID())
                    .contains(flux)) {
                    plugs.get(flux.networkID())
                        .add(flux);
                } else {
                    plugs.get(flux.networkID())
                        .remove(flux);
                ...
                if (!points.get(flux.networkID())
                    .contains(flux)) {
                    points.get(flux.networkID())
                        .add(flux);
                } else {
                    points.get(flux.networkID())
                        .remove(flux);
