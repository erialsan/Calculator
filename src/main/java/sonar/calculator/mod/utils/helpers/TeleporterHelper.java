...
public class TeleporterHelper {
            ...
            if (tile.dimension() != currentDimension) {
                if (!tile.getWorldObj().isRemote && !entity.isDead) {
                    ...
                    WorldServer worldServer = MinecraftServer.getServer()
                        .worldServerForDimension(tile.dimension());
                    MinecraftServer.getServer()
                        .getConfigurationManager()
                        .transferPlayerToDimension(
                            entityPlayerMP,
                            tile.dimension(),
                            new CalculatorTeleporter(
                                worldServer,
                                tile.xCoord + 0.5,
                                tile.yCoord - 2,
                                tile.zCoord + 0.5));
                    ...
                    if (currentDimension == 1) {
                        ((EntityPlayerMP) entity).playerNetServerHandler.setPlayerLocation(
                            ...
                            tile.zCoord + 0.5,
                            SonarHelper
                                .getAngleFromMeta(worldServer.getBlockMetadata(tile.xCoord, tile.yCoord, tile.zCoord)),
                            0);
    ...
    public static boolean canTeleport(TileEntityTeleporter target, TileEntityTeleporter current) {
        if ((target.xCoord != current.xCoord || target.yCoord != current.yCoord || target.zCoord != current.zCoord)
            && target.canTeleportPlayer()) {
