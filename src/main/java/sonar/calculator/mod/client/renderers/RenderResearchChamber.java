...
public class RenderResearchChamber extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity tileentity, double xCoord, double yCoord, double zCoord, float f) {
        RenderHelper.beginRender(
            ...
            yCoord + 1.5F,
            zCoord + 0.5F,
            RenderHelper.setMetaData(tileentity),
            texture.toString());
        ...
        if (tileentity.getWorldObj() != null) {
            ...
            if (chamber.slots[0] != null) {
                if (GameRegistry.findUniqueIdentifierFor(chamber.slots[0].getItem()).modId.equals(Calculator.modid)
                    && !RenderBlocks.renderItemIn3d(
                        Block.getBlockFromItem(chamber.slots[0].getItem())
                            .getRenderType())
                    && chamber.slots[0].getItem() instanceof ItemBlock
                    && Block.getBlockFromItem(chamber.slots[0].getItem())
                        .getRenderType() == -1
                    && MinecraftForgeClient.getItemRenderer(chamber.slots[0], ItemRenderType.FIRST_PERSON_MAP)
                        != null) {
                    ...
                    GL11.glRotated(180, 1, 0, 0);
                    if (!(chamber.slots[0].getItemSpriteNumber() == 0) || chamber.slots[0].getItemSpriteNumber() == 0
                        && chamber.slots[0].getItem() instanceof ItemBlock
                        && !RenderBlocks.renderItemIn3d(
                            Block.getBlockFromItem(chamber.slots[0].getItem())
                                .getRenderType())) {
        ...
        if (tileentity.getWorldObj() != null && tileentity instanceof TileEntityResearchChamber) {
            ...
            if (tile.slots[0] != null && tile.ticks != 0 && tile.ticks != tile.researchSpeed) {
                ...
                float f2 = (float) tileentity.getWorldObj()
