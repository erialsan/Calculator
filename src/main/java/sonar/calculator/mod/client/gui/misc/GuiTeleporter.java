...
public class GuiTeleporter extends GuiSonar {
            ...
            for (int i = start; i < finish; i++) {
                if (links.get(i) != null) {
                    if (links.get(i).networkID == entity.linkID) {
                        FontHelper
                            .text(links.get(i).networkName, 90, 7 + (12 * i) - (12 * start), Color.GREEN.getRGB());
                    } else {
        ...
        if (name.isFocused()) {
            if (c == 13 || c == 27) {
                ...
                if (text.isEmpty() || text == "" || text == null) {
                    SonarCore.network
                        .sendToServer(new PacketTextField("Unnamed", entity.xCoord, entity.yCoord, entity.zCoord, 1));
                } else {
                    SonarCore.network
                        .sendToServer(new PacketTextField(text, entity.xCoord, entity.yCoord, entity.zCoord, 1));
                }
                if (text.isEmpty() || text == "" || text == null) this.entity.name = "Unnamed";
                else this.entity.name = text;
            ...
            if (c == 13 || c == 27) {
                ...
                if (text.isEmpty() || text == "" || text == null) {
                    SonarCore.network
                        .sendToServer(new PacketTextField("", entity.xCoord, entity.yCoord, entity.zCoord, 2));
                } else {
                    SonarCore.network
                        .sendToServer(new PacketTextField(text, entity.xCoord, entity.yCoord, entity.zCoord, 2));
                }
                if (text.isEmpty() || text == "" || text == null) this.entity.password = "";
                else this.entity.password = text;
            ...
            if (c == 13 || c == 27) {
                ...
                if (text.isEmpty() || text == "" || text == null) {
                    SonarCore.network
                        .sendToServer(new PacketTextField("", entity.xCoord, entity.yCoord, entity.zCoord, 3));
                } else {
                    SonarCore.network
                        .sendToServer(new PacketTextField(text, entity.xCoord, entity.yCoord, entity.zCoord, 3));
                }
                if (text.isEmpty() || text == "" || text == null) this.entity.linkPassword = "";
                else this.entity.linkPassword = text;
    ...
    private boolean needsScrollBars() {
        if (entity.links == null) return false;
        if (entity.links.size() <= 4) return false;
        ...
        if (button.id >= 10) {
            if (entity.links != null) {
                ...
                if (network < entity.links.size()) {
                    ...
                    SonarCore.network
                        .sendToServer(new PacketByteBufServer(entity, entity.xCoord, entity.yCoord, entity.zCoord, 0));
                }
            }
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
        ...
        this.drawTexturedModalRect(
            scrollerLeft,
            scrollerStart + (int) ((float) (scrollerEnd - scrollerStart - 17) * this.currentScroll),
            176,
            0,
            10,
            15);
        if (entity.passwordMatch) this.drawTexturedModalRect(this.guiLeft + 162, this.guiTop + 65, 176, 15, 9, 14);
    }
