        ...
        if (!network()) {
            ...
            if (entity.networkName.equals("NETWORK")) {
                ...
                FontHelper.textCentre(
                    entity.networkName + ": " + getNetworkType(entity.networkState.getObject()),
                    xSize,
                    47,
                    0);
            }
    ...
    protected void keyTyped(char c, int i) {
        if (!network() && priority.isFocused()) {
            if (c == 13 || c == 27) {
                ...
                if (order.isEmpty() || order == "" || order == null) {
                    Calculator.network.sendToServer(
                        new PacketFluxPoint(String.valueOf(0), entity.xCoord, entity.yCoord, entity.zCoord, 1));
                } else {
                    Calculator.network
                        .sendToServer(new PacketFluxPoint(order, entity.xCoord, entity.yCoord, entity.zCoord, 1));
                }
                if (order.isEmpty() || order == "" || order == null) {
                    entity.priority = 0;
                } else {
            ...
            if (c == 13 || c == 27) {
                ...
                if (order.isEmpty() || order == "" || order == null) {
                    Calculator.network.sendToServer(
                        new PacketFluxPoint(String.valueOf(0), entity.xCoord, entity.yCoord, entity.zCoord, 2));
                } else {
                    Calculator.network
                        .sendToServer(new PacketFluxPoint(order, entity.xCoord, entity.yCoord, entity.zCoord, 2));
                }
