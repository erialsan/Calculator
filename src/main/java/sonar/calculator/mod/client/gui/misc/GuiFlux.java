...
public abstract class GuiFlux extends GuiSonar {
    ...
    public void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        List<FluxNetwork> networks = this.getNetworks();
        if (networks != null) {
            int start = (int) (this.getNetworks()
                .size() * this.currentScroll);
            int finish = Math.min(start + 4, networks.size());
            for (int i = start; i < finish; i++) {
                if (networks.get(i) != null) {
                    if (networks.get(i).networkName.equals(currentName)
                        && networks.get(i).networkID == getNetworkID()) {
                        FontHelper
                            .text(networks.get(i).networkName, 10, 10 + (12 * i) - (12 * start), Color.GREEN.getRGB());
                        FontHelper.text(
                            getNetworkType(networks.get(i).type),
                            105,
                            10 + (12 * i) - (12 * start),
                            Color.GREEN.getRGB());
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        Calculator.network.sendToServer(
            new PacketFluxPoint(
    ...
    protected void mouseClicked(int i, int j, int k) {
        ...
        if (!network()) {
            ...
            if (id.isFocused() && id.getText()
                .equals("NETWORK")) id.setText("");
        }
    ...
    protected void keyTyped(char c, int i) {
        if (network() && id.isFocused()) {
            if (c == 13 || c == 27) {
                ...
                if (text.isEmpty() || text == "" || text == null) this.setNetworkName("NETWORK");
                else this.setNetworkName(text);
    ...
    private boolean needsScrollBars() {
        if (this.getNetworks() == null) return false;
        if (this.getNetworks()
            .size() <= 4) return false;
    ...
    public class FluxButton extends SonarButtons.ImageButton {
        ...
        public FluxButton(int id, int x, int y) {
            super(
                ...
                x,
                y,
                new ResourceLocation("Calculator:textures/gui/buttons/buttons.png"),
                id == 0 ? 104 : id == 2 ? 68 : id == 3 ? 80 : 92,
                0,
                11,
                11);
            this.id = id;
