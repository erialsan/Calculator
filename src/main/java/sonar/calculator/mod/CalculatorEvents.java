...
public class CalculatorEvents {
     ...
     * if ((item != null) && (item.getItem().equals(Calculator.itemCraftingCalculator)) && ((player.openContainer
     * instanceof ICalculatorCrafter))) {
        ...
        if (event.block != null) {
            if (event.block == Calculator.AmethystSapling) {
                if (!event.world.isRemote) {
                    ((CalculatorSaplings) Calculator.AmethystSapling)
    ...
    public void Tanzanite(BonemealEvent event) {
        if (event.block != null) {
            if (event.block == Calculator.tanzaniteSapling) {
                if (!event.world.isRemote) {
                    ((CalculatorSaplings) Calculator.tanzaniteSapling)
                        .growTree(event.world, event.x, event.y, event.z, event.world.rand);
                    event.setResult(Result.ALLOW);
                }
            }
    ...
    public void pear(BonemealEvent event) {
        if (event.block != null) {
            if (event.block == Calculator.PearSapling) {
                if (!event.world.isRemote) {
                    ((CalculatorSaplings) Calculator.PearSapling)
                        .growTree(event.world, event.x, event.y, event.z, event.world.rand);
                    event.setResult(Result.ALLOW);
                }
            }
    ...
    public void diamond(BonemealEvent event) {
        if (event.block != null) {
            if (event.block == Calculator.diamondSapling) {
                if (!event.world.isRemote) {
                    ((CalculatorSaplings) Calculator.diamondSapling)
                        .growTree(event.world, event.x, event.y, event.z, event.world.rand);
                    event.setResult(Result.ALLOW);
                }
            }
