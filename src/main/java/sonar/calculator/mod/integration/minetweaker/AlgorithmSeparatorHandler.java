...
public class AlgorithmSeparatorHandler {
    ...
    public static void addRecipe(IIngredient input, IItemStack output1, IItemStack output2) {
        MineTweakerAPI.apply(
            new AddRecipeAction(input, MineTweakerMC.getItemStack(output1), MineTweakerMC.getItemStack(output2)));
    }
    ...
    private static class AddRecipeAction implements IUndoableAction {
        ...
        public AddRecipeAction(Object input, ItemStack output1, ItemStack output2) {
            ...
            if (input instanceof ILiquidStack) {
                MineTweakerAPI.logError(
                    "A liquid was passed intro a algorithm separator recipe, calculators do not use liquids when crafting, aborting!");
        ...
        @Override
        public void apply() {
            if (input == null || output1 == null || output2 == null) return;
            AlgorithmSeparatorRecipes.instance()
                .addRecipe(input, output1, output2);
        }
    ...
    private static class RemoveRecipeAction implements IUndoableAction {
        ...
        public RemoveRecipeAction(Object input) {
            ...
            if (input instanceof RecipeHelper.OreStack)
                dummyInput = OreDictionary.getOres(((RecipeHelper.OreStack) input).oreString)
                    .get(0);

            ItemStack[] stacks = AlgorithmSeparatorRecipes.instance()
                .getOutput(dummyInput);
            output1 = stacks[0];
            output2 = stacks[1];
        }
