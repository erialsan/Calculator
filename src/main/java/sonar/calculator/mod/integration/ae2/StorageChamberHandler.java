...
public class StorageChamberHandler {
    ...
    public static class ReflectionFactory {
        ...
        public static IMEMonitor<IAEItemStack> createStorageBusMonitor(IMEInventory<IAEItemStack> inventory,
    ...
    public static class StorageHandler implements IExternalStorageHandler {
        ...
        public IMEInventory getInventory(TileEntity te, ForgeDirection d, StorageChannel channel,
            ...
            if (te != null && te instanceof TileEntityStorageChamber) {
                return ReflectionFactory.createStorageBusMonitor(
                    new StorageInventory(te.getWorldObj(), te.xCoord, te.yCoord, te.zCoord),
                    src);
            }
            return null;
        }
    ...
    public static class StorageInventory implements IMEInventory<IAEItemStack> {
        ...
        public IAEItemStack injectItems(IAEItemStack input, Actionable type, BaseActionSource src) {
            ...
            if (chamber != null) {
                if (chamber.getSavedStack() != null) {
                    if (chamber.getCircuitType(input.getItemStack())
                        == chamber.getCircuitType(chamber.getSavedStack())) {
                        ...
                        if (stored + input.getStackSize() <= chamber.maxSize) {
                            ...
                            if (type != Actionable.SIMULATE) chamber.setStored(input.getItemDamage(), chamber.maxSize);
                    ...
                    if (type != Actionable.SIMULATE) {
                        chamber.setSavedStack(
                            input.getItemStack()
                                .copy());
                    }

                    if (input.getStackSize() <= chamber.maxSize) {
                        if (type != Actionable.SIMULATE) chamber.stored[input.getItemDamage()] += input.getStackSize();
                        return null;
                    } else {
                        if (type != Actionable.SIMULATE) chamber.stored[input.getItemDamage()] = chamber.maxSize;
        ...
        public IAEItemStack extractItems(IAEItemStack request, Actionable mode, BaseActionSource src) {
            ...
            if (chamber != null) {
                if (chamber.getSavedStack() != null) {
                    if (chamber.getCircuitType(request.getItemStack())
                        == chamber.getCircuitType(chamber.getSavedStack())) {
                        ...
                        if (stored != 0) {
                            if (stored <= request.getStackSize()) {
                                ...
                                ItemStack stack = chamber
                                    .getSlotStack(request.getItemDamage(), (int) request.getStackSize());
                                if (mode != Actionable.SIMULATE) {
