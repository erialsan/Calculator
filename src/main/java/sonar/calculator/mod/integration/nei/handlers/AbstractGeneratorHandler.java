...
public abstract class AbstractGeneratorHandler extends TemplateRecipeHandler {
    ...
    public void loadTransferRects() {
        ...
        this.transferRects.add(
            new TemplateRecipeHandler.RecipeTransferRect(
                new Rectangle(30 - 5, 41 - 11, 78, 10),
                "fuel",
                new Object[0]));
        this.transferRects.add(
            new TemplateRecipeHandler.RecipeTransferRect(
                new Rectangle(30 - 5, 17 - 11, 78, 10),
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if ((outputId.equals(getOverlayIdentifier()))
            && (AbstractGeneratorHandler.class.isAssignableFrom(getClass()))) {
    ...
    private static void findFuels() {
        ...
        for (ItemStack item : ItemList.items) if (!efuels.contains(item.getItem())) {
            int burnTime = TileEntityFurnace.getItemBurnTime(item);
            if (burnTime > 0) fuelList.add(new FuelPair(item.copy(), burnTime));
        }
    }
    ...
    public class ValuePair extends CachedRecipe {
        ...
        public List<PositionedStack> getOtherStacks() {
            return getCycledIngredients(
                AbstractGeneratorHandler.this.cycleticks / 48,
                Arrays.asList(new PositionedStack[] { this.starch }));
