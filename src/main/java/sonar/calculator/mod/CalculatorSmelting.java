...
public class CalculatorSmelting extends Calculator implements IFuelHandler {
    ...
    public int getBurnTime(ItemStack fuel) {
        if (fuel.getItem() == enriched_coal) return 5000;
        if (fuel.getItem() == coal_dust) return 1000;
        if (fuel.getItem() == purified_coal) return 10000;
        if (fuel.getItem() == firecoal) return 25000;
        if (fuel.getItem() == flawlessfirediamond) return 160000;
        if (fuel.getItem() == controlled_Fuel) return 80000;
