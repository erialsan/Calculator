...
public class SmeltingBlock extends SonarMachineBlock {
    ...
    public IIcon getIcon(IBlockAccess access, int x, int y, int z, int side) {
        ...
        if (entity != null) {
            if (entity instanceof TileEntityAbstractProcess) {
                ...
                if (side == metadata) {
                    return t.isActive() ? (t.getBlockTexture(side, metadata) ? this.frontActive : this.frontActive2)
                        : t.getBlockTexture(side, metadata) ? this.front : this.front2;
                }
    ...
    public IIcon getIcon(int side, int metadata) {
        return type < 4 || type == 6 ? this.blockIcon
            : side == metadata ? this.slot2
                : side == 0 ? this.slot1
                    : side == 1 ? this.slot1 : (metadata == 0) && (side == 3) ? this.front : this.slot1;
    }
    ...
    public void standardInfo(ItemStack stack, EntityPlayer player, List list) {
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard.isKeyDown(Keyboard.KEY_RSHIFT)) {
            ...
            list.add(
                FontHelper.translate("Energy Usage: ") + EnumChatFormatting.WHITE + energyUsage + " RF per operation");
