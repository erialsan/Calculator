        ...
        if (hasColours()) {
            ...
            for (int meta = 0; meta < 16; meta++) {
                ...
                for (int texID = 0; texID < 16; texID++) {
                    if (texID == 0) {
                        this.colours[texID + (meta * 16)] = iconRegister
                            .registerIcon(Calculator.modid + ":connected/" + type + colourType);
                    } else {
                        this.colours[texID + (meta * 16)] = iconRegister
                            .registerIcon(Calculator.modid + ":connected/" + type + "_" + texID + colourType);
            ...
            if (block1 instanceof ConnectedBlock) {
                ...
                if (block2 instanceof ConnectedBlock) {
                    for (int i = 0; i < connections1.length; i++) {
                        if (connections1[i] == ((ConnectedBlock) block2).target) return true;
                    }
                }
                if (block2 instanceof IConnectedBlock) {
                    ...
                    for (int i = 0; i < connections1.length; i++) {
                        for (int i2 = 0; i2 < connections2.length; i2++) {
                            if (connections1[i] == connections2[i2]) return true;
                        }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (target == 3 || target == 4) {
            if (player != null) {
                ...
                if (target != null
                    && (target.getItem() == Calculator.wrench || target.getItem() instanceof IToolHammer)) {
                    if (this.target == 3) {
