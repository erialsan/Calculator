    ...
    public boolean operateBlock(World world, int x, int y, int z, EntityPlayer player, BlockInteraction interact) {
        if (!world.isRemote) {
            ...
            if (held != null && held.getItem() instanceof IStability) {
                ...
                StoredItemStack stack = SonarAPI.getItemHelper()
                    .getStackToAdd(
                        1,
                        item,
                        SonarAPI.getItemHelper()
                            .addItems(tile, item.copy(), ForgeDirection.UP, ActionType.PERFORM, null));
                if (stack == null || stack.getStackSize() == 0) held.stackSize -= 1;
                return true;
            }
