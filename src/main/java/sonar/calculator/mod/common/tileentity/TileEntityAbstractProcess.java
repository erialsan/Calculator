...
public abstract class TileEntityAbstractProcess extends TileEntityProcess {
            ...
            if (output[o] == null) {
                ...
                if (slots[o + inputSize() + 1] != null) {
                    if (!slots[o + inputSize() + 1].isItemEqual(output[o])) {
                        return false;
                    } else if (slots[o + inputSize() + 1].stackSize + output[o].stackSize
                        > slots[o + inputSize() + 1].getMaxStackSize()) {
                            return false;
                        }
                }
            }
    ...
    public int getMaxOutputSize() {
        ...
        for (int o = 0; o < outputSize(); o++) {
            if (slots[o + inputSize() + 1] != null && output[o] != null) {
                size = Math.max(
                    (this.getInventoryStackLimit() - this.slots[o + inputSize() + 1].stackSize) / output[o].stackSize,
                    size);
            } else {
                size = 0;
            }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        if (slot < this.inputSize()) {
            if (recipeHelper() != null) {
                if (recipeHelper().validInput(stack)) return true;
            } else {
