...
public class TileEntityAdvancedGreenhouse extends TileEntityGreenhouse implements ISidedInventory {
    ...
    public final int requiredBuildEnergy = (stackStairs + stackLog + stackPlanks + stackGlass)
        * CalculatorConfig.getInteger("Build Energy");
        ...
        if (this.levelTicks == 20) {
            ...
            SonarAPI.getItemHelper()
                .transferItems(
                    this.getWorldObj()
                        .getTileEntity(
                            xCoord + (getForward().getOpposite().offsetX),
                            yCoord,
                            zCoord + (getForward().getOpposite().offsetZ)),
                    this,
        ...
        if (slots[0] != null && slots[1] != null
            ...
            if (slots[0].stackSize >= stackLog && checkLog(Block.getBlockFromItem(slots[0].getItem()))) {
                if (slots[1].stackSize + slots[2].stackSize + slots[3].stackSize >= stackStairs
                    && checkStairs(Block.getBlockFromItem(slots[1].getItem()))
                    && checkStairs(Block.getBlockFromItem(slots[2].getItem()))
                    && checkStairs(Block.getBlockFromItem(slots[3].getItem()))) {
                    if (slots[4].stackSize + slots[5].stackSize >= stackGlass
                        && checkGlass(Block.getBlockFromItem(slots[4].getItem()))
                        && checkGlass(Block.getBlockFromItem(slots[5].getItem()))) {
                        if (slots[6].stackSize >= stackPlanks
                            && checkPlanks(Block.getBlockFromItem(slots[6].getItem()))) {
                            return true;
                        }
                    }
                }
    ...
    public FailedCoords canMakeMultiblock() {
        ...
        int hoX = RenderHelper.getHorizontal(getForward())
            .getOpposite().offsetX;
        int hoZ = RenderHelper.getHorizontal(getForward())
            .getOpposite().offsetZ;
            ...
            if (start(false, this.worldObj, hX, hZ, hoX, hoZ, fX, fZ).getBoolean()) {
                if (sides(false, this.worldObj, hX, hZ, hoX, hoZ, fX, fZ, xCoord, yCoord, zCoord).getBoolean()) {
                    if (roof(false, this.worldObj, hX, hZ, hoX, hoZ, fX, fZ, xCoord, yCoord, zCoord).getBoolean()) {
                        if (underroof(false, this.worldObj, hX, hZ, hoX, hoZ, fX, fZ, xCoord, yCoord, zCoord)
                            .getBoolean()) {
                            addFarmland();
                            this.setCompleted();
                        }
    ...
    private void addFarmland() {
        ForgeDirection forward = ForgeDirection.getOrientation(this.getBlockMetadata())
            .getOpposite();
        ...
        for (int Z = -3; Z <= 3; Z++) {
            for (int X = -3; X <= 3; X++) {
                AgriCraftAPIWrapper.getInstance()
                    .removeWeeds(worldObj, x + X, y, z + Z, false);
                if (X == 3 && Z == 3) {
    ...
    public FailedCoords roof(boolean check, World w, int hX, int hZ, int hoX, int hoZ, int fX, int fZ, int x, int y,
        int z) {
        ...
        if (getPlanks(w.getBlock(x + (hX * 1), y + 6, z + (hZ * 1)))) {
            ...
            return new FailedCoords(
                false,
                x + (hX * 1),
                y + 6,
                z + (hZ * 1),
                FontHelper.translate("greenhouse.planks"));
        }
        ...
        for (int i = -1; i <= 9; i++) {
            for (int s = 3; s <= 7; s++) {
                ...
                if (getStairs(
                    w.getBlock(
                        x + (hX * intValues(s, FontHelper.translate("greenhouse.stairs"))) + (fX * i),
                        y + s,
                        z + (hZ * intValues(s, FontHelper.translate("greenhouse.stairs"))) + (fZ * i)))) {
                    if (!check) {
                        setStairs(
                            x + (hX * intValues(s, FontHelper.translate("greenhouse.stairs"))) + (fX * i),
                            y + s,
                    ...
                    return new FailedCoords(
                        ...
                        y + s,
                        z + (hZ * intValues(s, FontHelper.translate("greenhouse.stairs"))) + (fZ * i),
                        FontHelper.translate("greenhouse.stairs"));

                }
                if (getStairs(
                    w.getBlock(
                        x + (hoX * intValues(s, FontHelper.translate("greenhouse.stairs"))) + (fX * i),
                        y + s,
                        z + (hoZ * intValues(s, FontHelper.translate("greenhouse.stairs"))) + (fZ * i)))) {
                    if (!check) {
                        setStairs(
                            x + (hoX * intValues(s, FontHelper.translate("greenhouse.stairs"))) + (fX * i),
                            y + s,
    ...
    public FailedCoords underroof(boolean check, World w, int hX, int hZ, int hoX, int hoZ, int fX, int fZ, int x,
        int y, int z) {
    ...
    public FailedCoords sides(boolean check, World w, int hX, int hZ, int hoX, int hoZ, int fX, int fZ, int x, int y,
        int z) {
                    ...
                    if (s == 0) {
                        ...
                        if (getGlass(w.getBlock(x + (hX * 4) + (fX * i), y + s, z + (hZ * 4) + (fZ * i)))) {
                            ...
                            return new FailedCoords(
                                ...
                                y + s,
                                z + (hZ * 4) + (fZ * i),
                                FontHelper.translate("greenhouse.glass"));
                        }
        ...
        for (int Y = 0; Y <= 3; Y++) {
            ...
            if (getLog(w.getBlock(x + (hX * 4) + (fX * 4), y + Y, z + (hZ * 4) + (fZ * 4)))) {
                ...
                return new FailedCoords(
                    ...
                    y + Y,
                    z + (hZ * 4) + (fZ * 4),
                    FontHelper.translate("greenhouse.logs"));
            }
    ...
    public FailedCoords end(boolean check, World w, int hX, int hZ, int hoX, int hoZ, int fX, int fZ, int x, int y,
        int z) {
    ...
    public String getRequiredStacks() {
        ...
        if (slots[0] != null && slots[1] != null
            ...
            if (!checkStairs(Block.getBlockFromItem(slots[1].getItem()))) {
                return FontHelper.translate("greenhouse.stairsStack") + " 1 "
                    + FontHelper.translate("greenhouse.noStairs");
            }
            if (!checkStairs(Block.getBlockFromItem(slots[2].getItem()))) {
                return FontHelper.translate("greenhouse.stairsStack") + " 2 "
                    + FontHelper.translate("greenhouse.noStairs");
            }
            if (!checkStairs(Block.getBlockFromItem(slots[3].getItem()))) {
                return FontHelper.translate("greenhouse.stairsStack") + " 3 "
                    + FontHelper.translate("greenhouse.noStairs");
            }
            if (!checkGlass(Block.getBlockFromItem(slots[4].getItem()))) {
                return FontHelper.translate("greenhouse.glassStack") + " 1 "
                    + FontHelper.translate("greenhouse.noGlass");
            }
            if (!checkGlass(Block.getBlockFromItem(slots[5].getItem()))) {
                return FontHelper.translate("greenhouse.glassStack") + " 2 "
                    + FontHelper.translate("greenhouse.noGlass");
            }
            if (!checkPlanks(Block.getBlockFromItem(slots[6].getItem()))) {
                return FontHelper.translate("greenhouse.plankStack") + " "
                    + FontHelper.translate("greenhouse.noPlanks");
            }
            if (!(slots[0].stackSize >= stackLog)) {
                String logs = FontHelper.translate("greenhouse.requires") + " "
                    + (stackLog - slots[0].stackSize)
                    + " "
                    + FontHelper.translate("greenhouse.moreLogs");
                return logs;
            }
            if (!(slots[1].stackSize + slots[2].stackSize + slots[3].stackSize >= stackStairs)) {
                String stairs = FontHelper.translate("greenhouse.requires") + " "
                    + (stackStairs - slots[1].stackSize - slots[2].stackSize - slots[3].stackSize)
                    + " "
                    + FontHelper.translate("greenhouse.moreStairs");
                return stairs;
            }
            if (!(slots[4].stackSize + slots[5].stackSize >= stackGlass)) {
                String glass = FontHelper.translate("greenhouse.requires") + " "
                    + (stackGlass - slots[4].stackSize - slots[5].stackSize)
                    + " "
                    + FontHelper.translate("greenhouse.moreGlass");
                return glass;
            }
            if (!(slots[6].stackSize >= stackPlanks)) {
                String planks = FontHelper.translate("greenhouse.requires") + " "
                    + (stackPlanks - slots[6].stackSize)
                    + " "
                    + FontHelper.translate("greenhouse.morePlanks");
                return planks;
            }
        }
    ...
    public List<String> getWailaInfo(List<String> currenttip) {
        ...
        switch (this.isMulti) {
            case -1:
                currenttip
                    .add(FontHelper.translate("locator.state") + ": " + FontHelper.translate("greenhouse.building"));
                break;
            case 0:
                currenttip
                    .add(FontHelper.translate("locator.state") + ": " + FontHelper.translate("greenhouse.incomplete"));
                break;
            case 1:
                currenttip
                    .add(FontHelper.translate("locator.state") + ": " + FontHelper.translate("greenhouse.complete"));
                break;
            case 2:
                currenttip
                    .add(FontHelper.translate("locator.state") + ": " + FontHelper.translate("greenhouse.complete"));
