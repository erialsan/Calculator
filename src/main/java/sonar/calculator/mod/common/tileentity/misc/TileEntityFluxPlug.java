...
public class TileEntityFluxPlug extends TileEntityFluxHandler implements IFluxPlug {
        ...
        for (int i = 0; i < 6; i++) {
            if (this.handlers[i] != null) {
                if (handlers[i] instanceof IEnergyProvider) {
                    export -= ((IEnergyProvider) this.handlers[i])
                        .extractEnergy(ForgeDirection.VALID_DIRECTIONS[(i ^ 0x1)], export, simulate);
            ...
            for (int i = 0; i < points.size(); i++) {
                ...
                if (controller == null) {
                    if (target != null && target instanceof TileEntityFlux
                        && ((TileEntityFlux) (target)).dimension() == this.dimension()) {
                    ...
                    if (FluxHelper.checkPlayerName(target, networkID())) {
                        TransferList push = transferEnergy(
                            (IFluxPoint) target,
                            currentList,
                            controller.getRecieveMode(),
                            true);
                        currentList = push.inputList;
                        currentTransfer += push.energy;
                    }
    ...
    public TransferList sendEnergy(IFluxPoint point, int[] inputList, int recieveMode, boolean allowDimensions) {
        ...
        for (int i = 0; i < plugs.size(); i++) {
            ...
            if (target instanceof IFluxPlug) {
                ...
                int output = Math.min(
                    maxTransfer - point.pushEnergy(maxTransfer, true),
                    maxTransfer - ((IFluxPlug) target).pullEnergy(maxTransfer, true, true));
    ...
    public TransferList transferEnergy(IFluxPoint point, int[] inputList, int recieveMode, boolean allowDimensions) {
        ...
        for (int i = 0; i < plugs.size(); i++) {
            ...
            if (target != null
                && (allowDimensions
                    || target instanceof TileEntityFlux && ((IFluxPlug) target).dimension() == point.dimension())
                && FluxHelper.checkPlayerName(target, networkID())) {
                switch (recieveMode) {
                    case SURGE:
                        if (currentList[i] > 0) {
                            int limitedOutput = Math.min(
                                currentList[i] - point.pushEnergy(currentList[i], true),
                                currentList[i] - ((IFluxPlug) target).pullEnergy(currentList[i], true, true));
                            plugTransfer += push(point, ((IFluxPlug) target), limitedOutput, false);
                        }
                        break;
                    case HYPER_SURGE:
                        int o = 0;
                        int hyperSurge = Math.min(
                            maxOutput - point.pushEnergy(maxOutput, true),
                            maxOutput - ((IFluxPlug) target).pullEnergy(maxOutput, true, true));
                        while (o != 4) {
                    ...
                    default:
                        if (recieveMode == DISTRIBUTE) { // Even Distribution
                            int transfer = this.maxTransfer
                                - ((IFluxPlug) target).pullEnergy(this.maxTransfer, true, true);
                            int percentage = Math
                                .round((float) (maxOutput - point.pushEnergy(maxOutput, true)) / this.currentOutput);
    ...
    public int receiveEnergy(IFluxPoint point, int input, int recieveMode, boolean allowDimensions, boolean simulate) {
        ...
        if (allowDimensions || dimension() == point.dimension() && FluxHelper.checkPlayerName(this, networkID())) {
            switch (recieveMode) {
                case SURGE:
                    if (input > 0) {
                        int limitedOutput = Math
                            .min(input - point.pushEnergy(input, true), input - pullEnergy(input, true));
                ...
                case HYPER_SURGE:
                    int o = 0;
                    int hyperSurge = Math
                        .min(maxOutput - point.pushEnergy(maxOutput, true), maxOutput - pullEnergy(maxOutput, true));
                    while (o != 4) {
        ...
        if (maxTransfer == export) {
            ...
            if (target != null) {
                if (!networkName.equals("NETWORK")) {
                    return buffer
                        .receiveEnergy(Math.min(maxTransfer, ((TileEntityFluxPlug) target).currentOutput), simulate);
                }
            }
        }
    ...
    public boolean isMaster() {
        ...
        return flux.networkID() == networkID() && flux.dimension() == this.dimension()
            && flux.xCoord() == xCoord
            && flux.yCoord() == yCoord
            && flux.zCoord() == zCoord;
    }
