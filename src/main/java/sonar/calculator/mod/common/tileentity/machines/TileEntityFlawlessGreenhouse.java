...
public class TileEntityFlawlessGreenhouse extends TileEntityGreenhouse implements ISidedInventory, IFlawlessGreenhouse {
            ...
            for (int i = 0; i <= this.houseSize; i++) {
                if (this.storage.getEnergyStored() >= growthRF) {
                    if (growCrop(3, this.houseSize)) {
                        if (!this.worldObj.isRemote) plantsGrown++;
    ...
    public List<BlockCoords> getPlantArea() {
        ...
        for (int i = 0; i <= this.houseSize; i++) {
            for (int XZ = 1; XZ <= 2; XZ++) {
                coords.add(
                    new BlockCoords(xCoord + (hX * XZ) + (fX * (1 + i)), yCoord, zCoord + (hZ * XZ) + (fZ * (1 + i))));
    ...
    private int getGasAdd() {
        TileEntity tile = this.worldObj.getTileEntity(
            xCoord + (RenderHelper.getHorizontal(getForward()).offsetX * 3),
            yCoord,
            zCoord + (RenderHelper.getHorizontal(getForward()).offsetZ) * 3);
        ...
        for (int i = 0; i <= this.houseSize; i++) {
            for (int XZ = 1; XZ <= 2; XZ++) {
                if (this.worldObj.getBlock(
                    xCoord + (hX * XZ) + (fX * (1 + i)),
                    yCoord,
                    zCoord + (hZ * XZ) + (fZ * (1 + i))) instanceof IGrowable) {
                    this.plants++;
            ...
            for (int XZ = 0; XZ <= 3; XZ++) {
                if (XZ != 1 && XZ != 2) {
                    if (this.storage.getEnergyStored() >= waterRF) {
                        if (GreenhouseHelper.applyWater(
                            worldObj,
                            xCoord + (hX * XZ) + (fX * (1 + i)),
                            yCoord,
                            zCoord + (hZ * XZ) + (fZ * (1 + i)))) {
                    ...
                    if (this.storage.getEnergyStored() >= farmlandRF) {
                        if (GreenhouseHelper.applyFarmland(
                            worldObj,
                            xCoord + (hX * XZ) + (fX * (1 + i)),
                            yCoord,
                            zCoord + (hZ * XZ) + (fZ * (1 + i)))) {
    ...
    public FailedCoords checkSize(boolean check, World w, int hX, int hZ, int hoX, int hoZ, int fX, int fZ, int x,
        int y, int z) {
        this.houseSize = 0;
    ...
    public FailedCoords middle(boolean check, World w, int hX, int hZ, int hoX, int hoZ, int fX, int fZ, int x, int y,
        int z) {
        ...
        for (int i = 0; i <= 3; i++) {
            if (slabQuartz(x + (hX * i), y + 2, z + (hZ * i))) {
                return new FailedCoords(
                    ...
                    y + 2,
                    z + (hZ * i),
                    FontHelper.translate("greenhouse.quartz"));
            }
        ...
        for (int i = 0; i <= 3; i++) {
            if (stableStone(w.getBlock(x + (hX * i), y - 1, z + (hZ * i)))) {
                return new FailedCoords(
                    ...
                    y - 1,
                    z + (hZ * i),
                    FontHelper.translate("greenhouse.stable"));
            }
            if (slabQuartz(x + (hX * i), y + 2, z + (hZ * i))) {
                return new FailedCoords(
                    false,
