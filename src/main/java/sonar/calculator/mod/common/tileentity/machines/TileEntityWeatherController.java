...
public class TileEntityWeatherController extends TileEntityInventoryReceiver
    implements IEnergyHandler, IEnergySink, ISyncTile, IByteBufTile, IProcessMachine {
    ...
    public void startProcess() {
        ...
        if (buffer == 0 && coolDown == 0
            && storage.getEnergyStored() >= requiredPower
            && this.processType(type, true)
            ...
            case THUNDER:
                ...
                if (!simulate) {
                    this.worldObj.getWorldInfo()
                        .setRaining(data == 0 ? false : true);
                    this.worldObj.getWorldInfo()
                        .setThundering(data == 0 ? false : true);
                }
                return true;
        }
        return false;
