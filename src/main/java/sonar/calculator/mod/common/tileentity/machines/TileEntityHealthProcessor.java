...
public class TileEntityHealthProcessor extends TileEntitySidedInventory
    implements ISidedInventory, ISyncTile, IHealthProcessor {
    ...
    public void updateEntity() {
        super.updateEntity();
        if (!this.worldObj.isRemote) loot(slots[0]);
    ...
    private void loot(ItemStack stack) {
        if (!(stack == null)) {
            if (isLoot(stack)) {
                int add = (Integer) HealthProcessorRecipes.instance()
                    .getOutput(stack);
    ...
    private boolean isLoot(ItemStack stack) {
        if ((Integer) HealthProcessorRecipes.instance()
            .getOutput(stack) > 0) {
            return true;
        }
        return false;
