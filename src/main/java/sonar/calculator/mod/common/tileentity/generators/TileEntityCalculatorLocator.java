...
public class TileEntityCalculatorLocator extends TileEntityInventorySender implements IByteBufTile, ICalculatorLocator {
    ...
    public int currentOutput() {
        ...
        if (size != 0 && (((int) (2 * size + 1) * (2 * size + 1)) - 1) != 0) {
            ...
            return (int) (((5 + ((int) (1000 * (Math.sqrt(size * 1.8)) - 100 * (Math.sqrt(100 - stable)))
                / (int) (11 - Math.sqrt(stable))) * size)) * 2);
        }
        return 0;
    }
    ...
    public List<String> getWailaInfo(List<String> currenttip) {
        currenttip.add(
            FontHelper.translate("locator.active") + ": "
                + (!active.getObject() ? FontHelper.translate("locator.false") : FontHelper.translate("locator.true")));
        currenttip.add(
            FontHelper.translate("locator.owner") + ": "
                + (!owner.getObject()
                    .equals("None") ? owner.getObject() : FontHelper.translate("locator.none")));
        return currenttip;
    }
