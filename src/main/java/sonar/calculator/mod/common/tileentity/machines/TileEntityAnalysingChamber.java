...
public class TileEntityAnalysingChamber extends TileEntitySidedInventorySender
    implements ISidedInventory, IUpgradeCircuits, IAdditionalInventory {
    ...
    private void addEnergy() {
        ...
        if (SonarHelper.isEnergyHandlerFromSide(entity, ForgeDirection.DOWN)) {
            if (entity instanceof IEnergyHandler) {
                this.storage.modifyEnergyStored(
                    -((IEnergyHandler) entity)
                        .receiveEnergy(ForgeDirection.UP, this.storage.extractEnergy(maxTransfer, true), false));
            }
        }
    }
    ...
    private void analyse(int slot) {
        if (slots[slot].hasTagCompound()) {
            ...
            int storedEnergy = itemEnergy(
                slots[slot].getTagCompound()
                    .getInteger("Energy"));
            this.storage.receiveEnergy(storedEnergy, false);

            if (vUpgrade == 0) {
                ItemStack item1 = AnalysingChamberRecipes.instance()
                    .getResult(1, tag.getInteger("Item1"));
                ItemStack item2 = AnalysingChamberRecipes.instance()
                    .getResult(1, tag.getInteger("Item2"));
                if (item1 != null) {
                    add(item1, 2);
                }
                if (item2 == null) {
            ...
            ItemStack item3 = AnalysingChamberRecipes.instance()
                .getResult(2, tag.getInteger("Item3"));
            ItemStack item4 = AnalysingChamberRecipes.instance()
                .getResult(3, tag.getInteger("Item4"));
            ItemStack item5 = AnalysingChamberRecipes.instance()
                .getResult(4, tag.getInteger("Item5"));
            ItemStack item6 = AnalysingChamberRecipes.instance()
                .getResult(5, tag.getInteger("Item6"));
    ...
    public void readData(NBTTagCompound nbt, SyncType type) {
        ...
        if (type == SyncType.SAVE || type == SyncType.SYNC) {
            ...
            if (type == SyncType.SAVE) this.analysed = nbt.getShort("analysed");
        }
    }
    ...
    public void writeData(NBTTagCompound nbt, SyncType type) {
        ...
        if (type == SyncType.SAVE || type == SyncType.SYNC) {
            ...
            if (type == SyncType.SAVE) nbt.setShort("analysed", (short) this.analysed);
        }
