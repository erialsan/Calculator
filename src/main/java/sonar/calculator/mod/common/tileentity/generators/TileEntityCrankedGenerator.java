...
public class TileEntityCrankedGenerator extends TileEntitySender {
    ...
    public void updateEntity() {
        ...
        super.updateEntity();
        if (cranked()) {
            TileEntityCrankHandle crank = (TileEntityCrankHandle) this.worldObj
        ...
        for (int i = 0; i < 6; i++) {
            if (this.handlers[i] != null) {
                if (handlers[i] instanceof IEnergyReceiver) {
                    recieve -= ((IEnergyReceiver) this.handlers[i])
                        .receiveEnergy(ForgeDirection.VALID_DIRECTIONS[(i ^ 0x1)], recieve, simulate);
                } else if (handlers[i] instanceof IEnergySink) {
                    if (simulate) {
                        recieve -= ((IEnergySink) this.handlers[i]).getDemandedEnergy() * 4;
                    } else {
                        recieve -= (recieve - (((IEnergySink) this.handlers[i])
                            .injectEnergy(ForgeDirection.VALID_DIRECTIONS[(i ^ 0x1)], recieve / 4, 128) * 4));
                    }
                }
            }
        }
    ...
    public List<String> getWailaInfo(List<String> tooltip) {
        tooltip.add(
            FontHelper.translate("crank.cranked") + ": "
                + (this.cranked ? FontHelper.translate("locator.true") : FontHelper.translate("locator.false")));
        return tooltip;
    }
}
