...
public class TileEntityMagneticFlux extends TileEntityInventory
    implements IEntitySelector, ISidedInventory, IByteBufTile {
    ...
    public void magnetizeItems() {
        ...
        AxisAlignedBB aabb = AxisAlignedBB.getBoundingBox(
            ...
            yCoord + range,
            zCoord + range);
        List<EntityItem> items = this.worldObj.selectEntitiesWithinAABB(EntityItem.class, aabb, null);
    ...
    public boolean matchingStack(ItemStack stack, ItemStack stack2) {
        ...
        return stack.getItem() == stack2.getItem() && (exact || stack.getItemDamage() == stack2.getItemDamage())
            && (exact || ItemStack.areItemStackTagsEqual(stack, stack2));
    ...
    public ItemStack addToInventory(EntityItem item) {
        if (!this.worldObj.isRemote) {
            ...
            if (target instanceof IItemDuct) {
                ...
                itemstack = SonarAPI.getItemHelper()
                    .addItems(
                        target,
                        new StoredItemStack(itemstack),
