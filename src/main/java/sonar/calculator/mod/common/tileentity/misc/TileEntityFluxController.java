...
public class TileEntityFluxController extends TileEntityInventory implements IFluxController, ISyncTile, IByteBufTile {
                        ...
                        for (int i = 0; i < size; i++) {
                            ...
                            if (target != null) {
                                if (target.getItem() instanceof IEnergyContainerItem) {
                                    if (((IEnergyContainerItem) target.getItem()).getEnergyStored(target)
                                        != ((IEnergyContainerItem) target.getItem()).getMaxEnergyStored(target)) {
                                        recieve -= ((IEnergyContainerItem) target.getItem())
                                            .receiveEnergy(inv.getStackInSlot(i), recieve, simulate);
                                    }
                                    ...
                                    int itemEnergyRF = (int) Math.round(
                                        Math.min(
                                            Math.sqrt(item.getMaxCharge(target) * 4),
                                            (item.getMaxCharge(target) * 4) - (manager.getCharge(target) * 4)));
                                    int toTransferRF = Math.min(recieve, Math.round(itemEnergyRF));

                                    recieve -= manager.charge(target, toTransferRF / 4, 4, false, simulate) * 4;
                                } else
                                    if (SonarLoader.ic2Loaded() && target.getItem() instanceof ISpecialElectricItem) {
    ...
    public void setPlayer(EntityPlayer player) {
        ...
        this.playerName = player.getGameProfile()
            .getName();
    ...
    public void readPacket(ByteBuf buf, int id) {
        switch (id) {
            case 3:
                if (recieveMode < 4) recieveMode++;
                else recieveMode = 0;
                break;
            case 4:
                if (sendMode < 2) sendMode++;
                else sendMode = 0;
                break;
            case 5:
                if (transmitterMode < 2) transmitterMode++;
                else transmitterMode = 0;
                break;
            case 6:
                if (playerProtect < 2) playerProtect++;
                else playerProtect = 0;
                break;
        }
    }
