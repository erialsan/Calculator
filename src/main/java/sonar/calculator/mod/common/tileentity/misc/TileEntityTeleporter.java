...
public class TileEntityTeleporter extends TileEntitySonar implements ITeleport, IByteBufTile, ITextField {
    ...
    public void updateDimensionName(String name) {
        if (!destinationName.equals(name)) {
            ...
            SonarCore.network.sendToAllAround(
                new PacketTileSync(xCoord, yCoord, zCoord, syncData),
                new TargetPoint(worldObj.provider.dimensionId, xCoord, yCoord, zCoord, 32));
        }
    }
    ...
    public boolean canTeleportPlayer() {
        ...
        ForgeDirection[] dirs = new ForgeDirection[] { ForgeDirection.NORTH, ForgeDirection.SOUTH, ForgeDirection.EAST,
            ForgeDirection.WEST };
        int stable = 0;
        for (int i = 0; i < dirs.length; i++) {
            ...
            int blocks = 0;
            for (int j = 0; j < 3; j++) {
                if (worldObj.getBlock(xCoord + dir.offsetX, yCoord - j, zCoord + dir.offsetZ)
    ...
    public List<EntityPlayer> getPlayerList() {
        AxisAlignedBB aabb = AxisAlignedBB
            .getBoundingBox(xCoord - 1, yCoord - 2, zCoord - 1, xCoord + 1, yCoord - 1, zCoord + 1);
    ...
    public void textTyped(String string, int id) {
        if (id == 1) this.name = string;
        if (id == 2) this.password = string;
        if (id == 3) this.linkPassword = string;
