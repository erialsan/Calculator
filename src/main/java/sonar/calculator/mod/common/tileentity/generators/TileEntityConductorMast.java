...
public class TileEntityConductorMast extends TileEntityInventorySender implements ISidedInventory, IProcessMachine {
    ...
    public ItemStack recipeOutput(ItemStack stack) {
        return RecipeRegistry.ConductorMastItemRecipes.instance()
            .getCraftingResult(stack);
    }

    public int recipeEnergy(ItemStack stack) {
        return RecipeRegistry.ConductorMastPowerRecipes.instance()
            .getPowercost(stack);
    }
    ...
    public void updateEntity() {
        ...
        if (isServer()) {
            ...
            if (this.storage.getMaxEnergyStored() != this.storage.getEnergyStored()
                && this.storage.getEnergyStored() < storage.getMaxEnergyStored()
    ...
    private void addEnergy() {
        ...
        if (SonarHelper.isEnergyHandlerFromSide(entity, ForgeDirection.DOWN.getOpposite())) {
            this.storage.extractEnergy(
                SonarHelper.pushEnergy(entity, ForgeDirection.UP, this.storage.extractEnergy(maxTransfer, true), false),
                false);
        }
    }
    ...
    public int getStations() {
        ...
        for (int x = -10; x <= 10; x++) {
            for (int z = -10; z <= 10; z++) {
                if (this.worldObj.getTileEntity(xCoord + x, yCoord, zCoord + z) != null && this.worldObj
                    .getTileEntity(xCoord + x, yCoord, zCoord + z) instanceof TileEntityWeatherStation) {
                    TileEntityWeatherStation station = (TileEntityWeatherStation) this.worldObj
                        .getTileEntity(xCoord + x, yCoord, zCoord + z);
