...
public abstract class TileEntityGreenhouse extends TileEntityInventoryReceiver implements IGreenhouse {
    ...
    public static boolean canHarvest(World world, int x, int y, int z) {
        ...
        return block instanceof IGrowable || AgriCraftAPIWrapper.getInstance()
            .isMature(world, x, y, z);
    }
    ...
    public final boolean plant(ItemStack stack, int slot) {
        ...
        if (AgriCraftAPIWrapper.getInstance()
            .isHandledByAgricraft(stack)) {
            for (BlockCoords crops : coords) {
                if (AgriCraftAPIWrapper.getInstance()
                    .applySeeds(worldObj, crops.getX(), crops.getY(), crops.getZ(), stack)) {
                    this.planting = 0;
                    return true;
            ...
            for (BlockCoords crops : coords) {
                if (AgriCraftAPIWrapper.getInstance()
                    .placeCrops(worldObj, crops.getX(), crops.getY(), crops.getZ(), stack)) {
                    this.planting = 0;
                    return true;
    ...
    public void harvest(World world, int x, int y, int z) {
        ...
        List<ItemStack> agriArray = AgriCraftAPIWrapper.getInstance()
            .harvest(world, x, y, z);
            ...
            for (ItemStack stack : array) {
                if (stack != null) {
                    ...
                    StoredItemStack harvest = SonarAPI.getItemHelper()
                        .addItems(tile, storedstack.copy(), ForgeDirection.getOrientation(0), ActionType.PERFORM, null);
                    ...
                    if (!removed) world.setBlockToAir(x, y, z);
                    if (this.type == 3) this.plantsHarvested++;
                }
            }
    ...
    protected boolean canPlant(World worldObj, int x, int y, int z, int slot) {
        ...
        if (AgriCraftAPIWrapper.getInstance()
            .canPlaceCrops(worldObj, x, y, z, stack)) {
            return true;
        }
    ...
    public boolean checkGlass(Block block) {
        ...
        for (int i = 0; i < OreDictionary.getOres("blockGlass")
            .size(); i++) {
            if (OreDictionary.getOres("blockGlass")
                .get(i)
                .getItem() == Item.getItemFromBlock(block)) {
                return true;
            }
        }
        for (int i = 0; i < OreDictionary.getOres("blockGlassColorless")
            .size(); i++) {
            if (OreDictionary.getOres("blockGlassColorless")
                .get(i)
                .getItem() == Item.getItemFromBlock(block)) {
                return true;
            }
        }
        for (int i = 0; i < OreDictionary.getOres("paneGlassColorless")
            .size(); i++) {
            if (OreDictionary.getOres("paneGlassColorless")
                .get(i)
                .getItem() == Item.getItemFromBlock(block)) {
                return true;
            }
        }
        for (int i = 0; i < OreDictionary.getOres("paneGlass")
            .size(); i++) {
            if (OreDictionary.getOres("paneGlass")
                .get(i)
                .getItem() == Item.getItemFromBlock(block)) {
                return true;
            }
    ...
    public boolean checkStairs(Block block) {
        ...
        for (int i = 0; i < OreDictionary.getOres("stairWood")
            .size(); i++) {
            if (OreDictionary.getOres("stairWood")
                .get(i)
                .getItem() == Item.getItemFromBlock(block)) {
                return true;
            }
        }
        for (int i = 0; i < OreDictionary.getOres("stairStone")
            .size(); i++) {
            if (OreDictionary.getOres("stairStone")
                .get(i)
                .getItem() == Item.getItemFromBlock(block)) {
                return true;
            }
        }
        for (int i = 0; i < OreDictionary.getOres("greenhouse.stairs")
            .size(); i++) {
            if (OreDictionary.getOres("stairs")
                .get(i)
                .getItem() == Item.getItemFromBlock(block)) {
                return true;
            }
    ...
    public boolean checkPlanks(Block block) {
        ...
        for (int i = 0; i < OreDictionary.getOres("plankWood")
            .size(); i++) {
            if (OreDictionary.getOres("plankWood")
                .get(i)
                .getItem() == Item.getItemFromBlock(block)) {
                return true;
            }
        }
        for (int i = 0; i < OreDictionary.getOres("planksWood")
            .size(); i++) {
            if (OreDictionary.getOres("planksWood")
                .get(i)
                .getItem() == Item.getItemFromBlock(block)) {
                return true;
            }
