...
public class ContainerPortableDynamic extends Container implements ICalculatorCrafter {
    ...
    private static final int INV_START = FlawlessCalc.DynamicInventory.size, INV_END = INV_START + 26,
        HOTBAR_START = INV_END + 1, HOTBAR_END = HOTBAR_START + 8;

    public Map<Integer, Integer> research;

    public ContainerPortableDynamic(EntityPlayer player, InventoryPlayer inventoryPlayer, InventoryItem inventoryItem,
        ...
        this.addSlotToContainer(
            new SlotPortableCrafting(this, inventory, 0, 25, 9, isRemote, Calculator.itemFlawlessCalculator));
        this.addSlotToContainer(
            new SlotPortableCrafting(this, inventory, 1, 79, 9, isRemote, Calculator.itemFlawlessCalculator));
        this.addSlotToContainer(
            new SlotPortableResult(player, inventory, this, new int[] { 0, 1 }, 2, 134, 9, isRemote));

        this.addSlotToContainer(
            new SlotPortableCrafting(this, inventory, 3, 25, 35, isRemote, Calculator.itemFlawlessCalculator));
        this.addSlotToContainer(
            new SlotPortableCrafting(this, inventory, 4, 79, 35, isRemote, Calculator.itemFlawlessCalculator));
        this.addSlotToContainer(
            new SlotPortableResult(player, inventory, this, new int[] { 3, 4 }, 5, 134, 35, isRemote));

        addSlotToContainer(
            new SlotPortableCrafting(this, inventory, 6, 20 + 0 * 32, 61, isRemote, Calculator.itemFlawlessCalculator));
        addSlotToContainer(
            new SlotPortableCrafting(this, inventory, 7, 20 + 1 * 32, 61, isRemote, Calculator.itemFlawlessCalculator));
        addSlotToContainer(
            new SlotPortableCrafting(this, inventory, 8, 20 + 2 * 32, 61, isRemote, Calculator.itemFlawlessCalculator));
        this.addSlotToContainer(
    ...
    public int getCurrentUsage() {
        if (((Slot) this.inventorySlots.get(0)).getHasStack() || ((Slot) this.inventorySlots.get(1)).getHasStack()) {
            return 1;
        } else if (((Slot) this.inventorySlots.get(3)).getHasStack()
            || ((Slot) this.inventorySlots.get(4)).getHasStack()) {
                return 2;
            } else
            if (((Slot) this.inventorySlots.get(6)).getHasStack() || ((Slot) this.inventorySlots.get(7)).getHasStack()
