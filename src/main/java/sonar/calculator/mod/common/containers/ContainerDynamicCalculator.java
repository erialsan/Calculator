...
public class ContainerDynamicCalculator extends Container {
    ...
    private static final int INV_START = 10, INV_END = INV_START + 26, HOTBAR_START = INV_END + 1,
    ...
    public void onCraftMatrixChanged(IInventory iiventory) {
        this.calculatorResult.setInventorySlotContents(
            0,
            RecipeRegistry.CalculatorRecipes.instance()
                .getCraftingResult(calculatorMatrix.getStackInSlot(0), calculatorMatrix.getStackInSlot(1)));
        this.scientificResult.setInventorySlotContents(
            0,
            RecipeRegistry.ScientificRecipes.instance()
                .getCraftingResult(scientficMatrix.getStackInSlot(0), scientficMatrix.getStackInSlot(1)));
        this.atomicResult.setInventorySlotContents(
            0,
            RecipeRegistry.AtomicRecipes.instance()
                .getCraftingResult(
                    atomicMatrix.getStackInSlot(0),
                    atomicMatrix.getStackInSlot(1),
                    atomicMatrix.getStackInSlot(2)));
    ...
    public int getCurrentUsage() {
        if (((Slot) this.inventorySlots.get(1)).getHasStack() || ((Slot) this.inventorySlots.get(2)).getHasStack()) {
            return 1;
        } else if (((Slot) this.inventorySlots.get(4)).getHasStack()
            || ((Slot) this.inventorySlots.get(5)).getHasStack()) {
                return 2;
            } else
            if (((Slot) this.inventorySlots.get(7)).getHasStack() || ((Slot) this.inventorySlots.get(8)).getHasStack()
