...
public class FlawlessCalc extends SonarCalculator implements IItemInventory, IResearchStore {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean par4) {
        ...
        if (stack.hasTagCompound()) {
            int max = stack.getTagCompound()
                .getInteger("Max");
            int stored = stack.getTagCompound()
                .getInteger("Stored");
            if (max != 0) {
        ...
        if (player.isSneaking()) {
            ...
            switch (mode) {
                case FlawlessCraft:
                    if (!world.isRemote) {
                        player
                            .openGui(Calculator.instance, CalculatorGui.FlawlessCalculator, world, -1000, -1000, -1000);
                    }
                    break;
                case DynamicCraft:
                    if (!world.isRemote) {
                    ...
                    if (CalculatorConfig.enableGrenades) {
                        if (!world.isRemote && player.capabilities.isCreativeMode) {
                            ...
                            player.addChatComponentMessage(
                                new ChatComponentText(FontHelper.translate("energy.notEnough")));
                        }
                    } else {
        ...
        if (world.getBlock(x, y, z) == Calculator.stablestoneBlock) {
            if (stack.hasTagCompound()) {
                if (mode == FlawlessCalc.Teleport) {
                    if ((world.getBlock(x, y, z) == Calculator.stablestoneBlock) && (!stack.getTagCompound()
                        .getBoolean("Tele"))) {
                        stack.getTagCompound()
                            .setDouble("TeleX", x - 0.5);
                        stack.getTagCompound()
                            .setDouble("TeleY", y + 1);
                        stack.getTagCompound()
                            .setDouble("TeleZ", z - 0.5);
                        stack.getTagCompound()
                            .setBoolean("Tele", true);
                        stack.getTagCompound()
                            .setInteger("Dimension", player.dimension);
                        if (!world.isRemote) {
                            player.addChatComponentMessage(
                    ...
                    } else if ((world.getBlock(x, y, z) == Calculator.stablestoneBlock) && (stack.getTagCompound()
                        .getBoolean("Tele"))
                        && (world.getBlock(
                            (int) stack.getTagCompound()
                                .getDouble("TeleX"),
                            (int) stack.getTagCompound()
                                .getDouble("TeleY") - 1,
                            (int) stack.getTagCompound()
                                .getDouble("TeleZ"))
                            != Calculator.stablestoneBlock)) {
                                stack.getTagCompound()
                                    .setDouble("TeleX", x + 0.5);
                                stack.getTagCompound()
                                    .setDouble("TeleY", y + 1);
                                ...
                                if (!world.isRemote) {
                                    player.addChatComponentMessage(
                                        new ChatComponentText(FontHelper.translate("calc.position")));
                                }
    ...
    public void teleport(EntityPlayer player, World world, ItemStack stack) {
        if (player.dimension == stack.getTagCompound()
            .getInteger("Dimension")) {
            if (stack.getTagCompound()
                .getBoolean("Tele")) {
                Block target = world.getBlock(
                    (int) (stack.getTagCompound()
                        .getDouble("TeleX") - 0.5),
                    (int) stack.getTagCompound()
                        .getDouble("TeleY") - 1,
        ...
        if (!world.isRemote && player.dimension != stack.getTagCompound()
            .getInteger("Dimension")) {
    ...
    public Map<Integer, Integer> getResearch(ItemStack stack) {
        ...
        if (stack != null && stack.getItem() instanceof IResearchStore) {
            ...
            unblocked = CalculatorRecipes.instance()
                .readFromNBT(stack.getTagCompound(), "unblocked");
        }
        return unblocked;
    }
    ...
    public void setResearch(ItemStack stack, Map<Integer, Integer> unblocked, int stored, int max) {
        if (stack != null && stack.getItem() == Calculator.itemCalculator) {
            ...
            CalculatorRecipes.instance()
                .writeToNBT(stack.getTagCompound(), unblocked, "unblocked");
            stack.getTagCompound()
                .setInteger("Max", max);
            stack.getTagCompound()
                .setInteger("Stored", stored);
