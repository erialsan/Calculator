...
public class NutritionModule extends SonarItem implements IHealthStore, IHungerStore {
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int par, boolean bool) {
        if (!world.isRemote) {
            if (entity instanceof EntityPlayer) {
                ...
                int ticks = stack.getTagCompound()
                    .getInteger("ticks");
                if (ticks < 10) {
                    stack.getTagCompound()
                        .setInteger("ticks", ticks + 1);
                } else {
                    stack.getTagCompound()
                        .setInteger("ticks", 0);
                    int points = stack.getTagCompound()
                        .getInteger("hunger");
    ...
    @Override
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int par,
